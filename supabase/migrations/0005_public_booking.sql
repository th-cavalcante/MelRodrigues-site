-- MR Laser — Agenda Online (agendamento público, sem login)
-- Rodar no SQL Editor do projeto Supabase (Dashboard > SQL Editor > New query).

-- Garante que dois pacientes não reservem o mesmo horário na mesma sala
-- (cancelados liberam o horário de novo).
create unique index if not exists bookings_room_slot_uidx on public.bookings (room, booking_date, booking_time)
  where status <> 'cancelado';

-- 1) nav da página pública mostra "Olá, <nome>" — só o nome, nada mais.
create or replace function public.get_patient_name(p_patient_id uuid)
returns text
language sql
security definer
set search_path = public
as $$
  select name from public.patients where id = p_patient_id;
$$;

revoke all on function public.get_patient_name from public;
grant execute on function public.get_patient_name to anon, authenticated;

-- 2) horários já ocupados no dia, pra não deixar escolher um horário
-- que outra pessoa já reservou. Só devolve o horário, nada sobre quem.
...
as $$
  ...
  where booking_date = p_date
    and room = 'Sala Laser Hakon 4D'
    and status <> 'cancelado';
$$;

revoke all on function public.get_booked_times from public;
grant execute on function public.get_booked_times to anon, authenticated;

-- 3) paciente confirma o agendamento pelo link público.
...
begin
  ...
  -- mesma lógica de deriveHealthAlert() em src/lib/agendaConstants.js
  ...
  -- array_length() devolve NULL (não 0) pra array vazio — coalesce evita
  -- "null value in column health_alert violates not-null constraint".
